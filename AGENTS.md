...
- Physical-host qualification requires both the existing signed execution authorization and a separate fresh, digest-bound exclusive-host lease. A single locked console is eligible only when its signed v2 lease reports zero interactive AO workloads; unlocked, unknown, or busy sessions remain ineligible. Never treat the lease as authority for arbitrary commands, broad process termination, graphical-session mutation, or cleanup outside its lease-scoped scratch root.
