...
- Treat root quality manifests as untrusted source-owned declarations. Bind commit checks to the Git index, push checks to the exact base/head range, execute only validated argv arrays, and fail when a selected command changes Git state.
