...
- Release-sensitive physical qualification requires both the existing signed execution authorization and a fresh, digest-bound exclusive-host lease. Fixed lifecycle checks use bounded shared leases: unrelated interactive, Codex, IDE, and SSH activity is allowed, while conflicting lease, workload, scratch, action, or resource state fails closed. Never treat either lease as authority for arbitrary commands, broad process termination, graphical-session mutation, or cleanup outside its lease-scoped scratch root.
