...
- Release-sensitive physical qualification requires both the existing signed execution authorization and a fresh, digest-bound bounded lease. Unrelated interactive, Codex, IDE, and SSH activity is allowed, while conflicting lease, workload, scratch, or resource state fails closed. Keep legacy exclusive-host evidence import-compatible. Never treat either lease as authority for arbitrary commands, broad process termination, graphical-session mutation, or cleanup outside its lease-scoped scratch root.
- Keep physical-Windows repository commands fixed in the worker-owned profile map; AO Next qualification runs only its locked Cargo workspace tests and release build.
- Keep the packaged Windows worker and `ao2-windows-worker.cmd` launcher Windows-archive-only. Require Python 3.11 or newer, preserve argument boundaries and offline validation, and never treat archive presence as execution or provider authority.
