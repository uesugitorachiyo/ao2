...
# ao2.release-provenance.v1
# release_provenance_sign=passed is emitted by the native AO2 command.
...
AO2_RELEASE_PRIVATE_KEY="${AO2_RELEASE_PRIVATE_KEY:-.release-signing/ao2-release-signing-key.pem}"
...
set -- "$@" \
  ...
  --private-key "$AO2_RELEASE_PRIVATE_KEY" \
  --release-tag "$AO2_RELEASE_TAG"
