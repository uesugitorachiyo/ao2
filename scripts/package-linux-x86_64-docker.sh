...
if [ -n "$AO2_LINUX_X86_64_SSH_TARGET" ]; then
  ...
  ssh "$AO2_LINUX_X86_64_SSH_TARGET" "cd '$remote_src' && export PATH=\"\$HOME/.cargo/bin:\$PATH\" && cargo build --release -p ao2-cli && target/release/ao2 release package --out-dir dist-linux-x86_64 --target-label linux-x86_64 && archive='$remote_dist/ao2-${VERSION}-linux-x86_64.tar.gz' && test -f \"\$archive\" && tmp=\$(mktemp -d) && tar -xzf \"\$archive\" -C \"\$tmp\" && \"\$tmp/bin/ao2\" --help >/dev/null && test -x \"\$tmp/install.sh\" && grep -q 'bin/ao2' \"\$tmp/SHA256SUMS\" && sha256sum \"\$archive\""
