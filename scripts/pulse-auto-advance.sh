...
def check_status(check: dict) -> str:
    ...
    if conclusion in {"failure", "cancelled", "timed_out", "action_required"}:
        return "blocked"
    if status in {"failure", "failed", "cancelled", "timed_out", "action_required"}:
        return "blocked"
...
def load_pr_ci_gate(trigger: str) -> dict:
    gate = {
        "schema_version": "ao2.pulse-pr-ci-gate.v1",
        "enabled": pr_ci_gate_enabled,
        "state_path": str(pr_ci_gate_state),
    ...
    if not pr_ci_gate_enabled:
        gate["reason"] = "gate_disabled"
        return gate
    if not pr_ci_gate_state.is_file():
        return gate
    try:
        state = json.loads(pr_ci_gate_state.read_text(encoding="utf-8"))
    except json.JSONDecodeError as exc:
        gate["status"] = "waiting"
        gate["reason"] = "waiting_for_pr_merge_or_ci"
        gate["detail"] = f"gate_state_invalid_json: {exc}"
        return gate
    if not isinstance(state, dict):
        gate["status"] = "waiting"
        gate["reason"] = "waiting_for_pr_merge_or_ci"
        gate["detail"] = "gate_state_not_object"
        return gate

    gate["source_schema_version"] = state.get("schema_version")
    gate["branch"] = state.get("branch")
    pr = state.get("pr")
    ...
    if state_status in {"waiting", "pending", "failed", "blocked", "open", "draft"}:
        gate["status"] = "waiting"
        gate["reason"] = "waiting_for_pr_merge_or_ci"
        gate.setdefault("detail", f"gate_status_{state_status}")
    ...
    if isinstance(required_checks, list):
        ...
        for check in required_checks:
            if not isinstance(check, dict):
                gate["status"] = "waiting"
                gate["reason"] = "waiting_for_pr_merge_or_ci"
                gate["detail"] = "required_check_not_object"
                continue
            check_gate_status = check_status(check)
            if check_gate_status != "passed":
                gate["status"] = "waiting"
                gate["reason"] = "waiting_for_pr_merge_or_ci"
                gate.setdefault("detail", f"required_check_{check_gate_status}")
    else:
        gate["status"] = "waiting"
        gate["reason"] = "waiting_for_pr_merge_or_ci"
        gate["detail"] = "required_checks_not_list"

    if gate["status"] == "passed" and state_status in {"passed", "green", "ready", "merged", "closed_success"}:
        gate["reason"] = "passed"
    return gate
    ...
    if eval_loop_sha256 in seen and not allow_duplicate:
        if forever:
            if pulse_generate_next("duplicate_eval_loop_digest"):
                ...
                continue
            if payload.get("reason") == "waiting_for_pr_merge_or_ci":
                break
