...
def test_public_release_links_and_install_guide_track_current_stable():
    ...
    for needle in [
        "https://github.com/uesugitorachiyo/ao2/releases/tag/v0.5.5",
        "https://github.com/uesugitorachiyo/ao2/releases/download/v0.5.5",
        "img.shields.io/github/v/release/uesugitorachiyo/ao2",
        "gh release download v0.5.5 --repo uesugitorachiyo/ao2",
        "ao2-0.5.5-macos-aarch64.tar.gz",
        "ao2-0.5.5-linux-x86_64.tar.gz",
        "ao2-0.5.5-windows-x86_64.tar.gz",
        "SHA256SUMS",
    ]:
        assert needle in readme

    assert "The current stable public release line is `v0.5.5`." in install
...
def test_current_public_pair_tracks_control_plane_v0_1_18():
    ...
    expected_next_patch = {
        "ao2": {"tag": "v0.5.6", "version": "0.5.6"},
        "ao2_control_plane": {"tag": "v0.1.18", "version": "0.1.18"},
        "promotion_confirm": "promote-stable-v0.5.6-v0.1.18",
        "public_operator_confirm": "public-release-reviewed-v0.5.6-v0.1.18",
    }
...
def test_public_release_download_verify_is_checksum_first_and_post_merge_canaried():
    ...
    for needle in [
        ...
        "stable public release archives at v0.5.5",
    ]:
...
def test_public_release_pair_digest_audit_rejects_closure_release_asset_drift(tmp_path):
    ...
    ao2_assets = [
        ("ao2-0.5.5-linux-x86_64.tar.gz", "d" * 64, 3345603),
        ("ao2-0.5.5-macos-aarch64.tar.gz", "e" * 64, 3345605),
        ("ao2-0.5.5-windows-x86_64.tar.gz", "f" * 64, 3345607),
    ]
...
def test_public_release_pair_digest_audit_rejects_missing_or_mismatched_full_archive_parity(tmp_path):
    ...
    ao2_archives = {
        "ao2-0.5.5-linux-x86_64.tar.gz": ("b" * 64, 102),
        "ao2-0.5.5-macos-aarch64.tar.gz": ("c" * 64, 103),
        "ao2-0.5.5-windows-x86_64.tar.gz": ("d" * 64, 104),
    }
    cp_archives = {
    ...
    assert any(
        ...
        and item["code"] == "required_archive_presence"
            and "ao2-0.5.5-windows-x86_64.tar.gz" in item["missing_assets"]
...
def test_release_train_manifest_centralizes_stable_and_next_patch_defaults():
    ...
    assert exported["AO2_RELEASE_TRAIN_AO2_TAG"] == "v0.5.6"
    assert exported["AO2_RELEASE_TRAIN_AO2_VERSION"] == "0.5.6"
    ...
    assert exported["AO2_RELEASE_TRAIN_PUBLIC_OPERATOR_CONFIRM"] == (
        "public-release-reviewed-v0.5.6-v0.1.18"
    )
...
def test_candidate_patch_release_rehearsal_audit_contract(tmp_path):
    ...
    assert audit["release_targets"]["ao2"]["tag"] == "v0.5.6"
...
def test_release_train_manifest_parity_audit_contract(tmp_path):
    ...
    assert summary["stable"]["ao2"]["tag"] == "v0.5.5"
    assert summary["next_patch"]["ao2"]["tag"] == "v0.5.6"
...
def test_release_immutability_audit_composes_stable_asset_and_download_checks():
    ...
    assert "v0.5.5" in readme
    assert "ao2-0.5.5-linux-x86_64.tar.gz" in readme
...
set -eu
if [ "${1:-}" = "version" ] && [ "${2:-}" = "--json" ]; then
  printf '{"package":"ao2","version":"0.5.5","target":"linux-x86_64","release_manifest_schema":"ao2.release-manifest.v1"}\n'
  exit 0
fi
if [ "${1:-}" = "--help" ]; then
  printf 'ao2 help\n'
...
""",
    ...
    assert summary["release_pair"]["ao2"]["tag"] == "v0.5.5"
