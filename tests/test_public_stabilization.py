...
def test_ci_reports_legacy_non_approval_required_check_names():
    ...
    assert "cargo test -p ao2-cli --test cli_report_cockpit" in ci
    ...
    assert "cargo test -p ao2-cli --test cli_plugin_package" in ci
    assert "cargo test -p ao2-cli --test cli_plugin_consumer_lifecycle" in ci
    assert "cargo test -p ao2-cli --test cli_plugin_release_candidate" in ci
    assert "cargo test -p ao2-cli --test cli_plugin_distribution" in ci
    assert "cargo test -p ao2-cli --test cli_plugin_adapter" in ci
    assert "cargo test -p ao2-cli --test cli_plugin_wrapper_harness" in ci
...
def test_next_patch_release_notes_contract_resolves_v0_5_13_note():
    result = subprocess.run(
        [
            "python3",
    ...
    assert Path(result.stdout.strip()).as_posix() == "docs/release/v0.5.13-stable.md"
...
def test_v0_5_12_windows_worker_release_contract():
    notes = read("docs/release/v0.5.12-stable.md")
    documentation = "\n".join(
        read(path)
        for path in (
            "README.md",
            ...
            "docs/windows-outbound-worker.md",
        )
    )
    assert "ao2-windows-worker.cmd" in notes
    assert "Python 3.11" in notes
    assert "Windows archive" in notes
    assert "Linux and macOS archives remain unchanged" in notes
    assert "ao2-windows-worker.cmd" in documentation
    assert "Python 3.11" in documentation


def test_v0_5_13_source_growth_release_contract():
    release_train = json.loads(read("docs/release/release-train.json"))

    assert release_train["stable"]["ao2"] == {
        "tag": "v0.5.13",
        "version": "0.5.13",
    }
    assert release_train["next_patch"]["ao2"] == {
        "tag": "v0.5.13",
        "version": "0.5.13",
    }
    notes = read("docs/release/v0.5.13-stable.md")
    for required in [
        "context.software_source_policy",
        "Agent-backed SDD execution",
        "Generated and vendored source",
        "non-source artifacts remain outside",
        "base/head",
        "quality snapshots",
        "does not add a universal language parser",
    ]:
...
def test_public_release_build_uses_canonical_hosted_native_dry_run_contract():
    ...
    for needle in [
        ...
        "Authenticate producer workflow run and artifact",
        "validate_physical_windows_workflow_run.py validate-run-id",
        "validate_physical_windows_workflow_run.py validate-metadata",
...
def test_public_release_links_and_install_guide_track_current_stable():
    ...
    stable = json.loads(read("docs/release/release-train.json"))["stable"]["ao2"]

    assert stable == {"tag": "v0.5.13", "version": "0.5.13"}

    for needle in [
        f"https://github.com/uesugitorachiyo/ao2/releases/tag/{stable['tag']}",
        f"https://github.com/uesugitorachiyo/ao2/releases/download/{stable['tag']}",
        "img.shields.io/github/v/release/uesugitorachiyo/ao2",
        f"gh release download {stable['tag']} --repo uesugitorachiyo/ao2",
        f"ao2-{stable['version']}-macos-aarch64.tar.gz",
        f"ao2-{stable['version']}-linux-x86_64.tar.gz",
        f"ao2-{stable['version']}-windows-x86_64.tar.gz",
        "SHA256SUMS",
    ]:
    ...
    for path, text in [
        ("README.md", readme),
        ("docs/FIRST-30-MINUTES.md", first_30_minutes),
    ...
    assert f"The installed version should report `{stable['version']}`." in first_30_minutes
    assert f"The current stable public release line is `{stable['tag']}`." in install
    assert "v0.5.5" not in first_30_minutes
...
def test_published_v0_5_13_is_current_operational_release_truth():
    release_train = json.loads(read("docs/release/release-train.json"))
    expected_pair = {
    ...
    assert release_train["stable"] == expected_pair
    assert release_train["next_patch"] == expected_pair
    ...
    for path in (
        ...
        text = read(path)
        assert "v0.5.13" in text, path
        assert "ao2-0.5.13" in text, path

    assert "The next Windows patch archive" not in read("README.md")
...
def test_current_public_pair_tracks_control_plane_v0_1_19():
    ...
    vector = json.loads(
        read("tests/fixtures/compatibility/ao2-execution-receipt-v0.5.2.json")
    )
    ...
    assert "AO2 Control Plane v0.1.17" in release_notes
...
def test_public_release_pair_digest_audit_rejects_closure_release_asset_drift(tmp_path):
    ...
    ao2_assets = [
        ("ao2-0.5.13-linux-x86_64.tar.gz", "d" * 64, 3345603),
        ("ao2-0.5.13-macos-aarch64.tar.gz", "e" * 64, 3345605),
        ("ao2-0.5.13-windows-x86_64.tar.gz", "f" * 64, 3345607),
    ]
        ...
        json.dumps(
            {
                ...
                "url": "https://github.com/uesugitorachiyo/ao2/releases/tag/v0.5.13",
                "assets": [
...
def test_public_release_pair_digest_audit_rejects_missing_or_mismatched_full_archive_parity(tmp_path):
    ...
    ao2_archives = {
        "ao2-0.5.13-linux-x86_64.tar.gz": ("b" * 64, 102),
        "ao2-0.5.13-macos-aarch64.tar.gz": ("c" * 64, 103),
        "ao2-0.5.13-windows-x86_64.tar.gz": ("d" * 64, 104),
    }
    cp_archives = {
    ...
    def release_fixture(path: Path, component: str, archives: dict[str, tuple[str, int]]):
        path.write_text(
            json.dumps(
                {
                    "tagName": "v0.5.13" if component == "ao2" else "v0.1.19",
                    "name": (
                        "AO2 v0.5.13"
                        if component == "ao2"
    ...
    assert any(
        ...
        and item["code"] == "required_archive_presence"
            and "ao2-0.5.13-windows-x86_64.tar.gz" in item["missing_assets"]
    ...
    assert passed_summary["archive_parity"]["components"]["ao2"]["required_archive_count"] == 3
    assert (
...
def test_stable_promotion_workflow_is_guarded_and_documented():
    ...
    for needle in [
        ...
        "AO2_STABLE_PROMOTION_CLOSURE_INDEX_RUN_ID",
        ...
        json.dumps(
            {
                ...
                "status": "passed",
                "ao2": {"release_repo": "uesugitorachiyo/ao2", "release_tag": "v0.4.81"},
                "control_plane": {
...
def test_operator_release_evidence_bundle_downloads_and_verifies_cross_repo_artifacts(tmp_path):
    ...
    for needle in [
        ...
        "AO2_OPERATOR_RELEASE_CLOSURE_INDEX_RUN_ID",
        "gh run list --repo \"$repo\" --branch main --workflow \"$workflow\" --status success",
        "gh run download \"$run_id\" --repo \"$repo\" --name \"$artifact\" --dir \"$dest\"",
        "ao2-dual-repo-release-publication-closure-index",
        "post-stable-release-smoke-Linux",
...
def test_stable_release_evidence_packet_combines_release_and_operator_baselines(tmp_path):
    ...
    for needle in [
        ...
        "AO2_STABLE_PROMOTION_CLOSURE_INDEX_RUN_ID=${{ github.run_id }}",
        "npm run release:stable-promotion-workflow",
        "AO2_OPERATOR_RELEASE_EVIDENCE_ROOT=target/stable-release-evidence-packet-ci/operator-release-evidence-bundle",
        "AO2_OPERATOR_RELEASE_CLOSURE_INDEX_RUN_ID=${{ github.run_id }}",
        "npm run release:operator-evidence-bundle",
...
def test_release_readiness_static_gate_locks_cross_os_ci_contract(tmp_path):
    ...
    for needle in [
        ...
        "AO2_RELEASE_CANONICAL_HOSTED_SUMMARY=target/canonical-hosted-public-release-verification.json",
        ...
        "ao2-dual-repo-release-publication-closure-index/summary.json",
        {
            ...
            "control_plane": {
                ...
                "assets": [
                    {"name": "SHA256SUMS"},
                    {
                        "name": "ao2-control-plane-0.1.14-linux-x86_64.tar.gz",
                        "sha256": "a" * 64,
                        "size_bytes": 4096,
                    },
                ],
...
def test_ci_runs_python_guard_tests_and_pulse_docs_reference_persistent_local_mirror():
    ...
    assert (
        ...
        "tests/test_ao2_native_runtime_platform_evidence.py "
            ...
            "tests/test_ao2_rsi_live_self_change_rehearsal.py "
            "tests/test_windows_outbound_worker.py -q"
        ) in ci
...
def test_no_factory_v3_guard_allows_decomposed_skill_contract_manifest_source(tmp_path):
    repo = tmp_path / "repo"
    (repo / "scripts").mkdir(parents=True)
    (repo / "crates" / "ao2-cli" / "src").mkdir(parents=True)
    (repo / "crates" / "ao2-runtime" / "src").mkdir(parents=True)
    (repo / "package.json").write_text('{"scripts": {}}\n', encoding="utf-8")
    (repo / "crates" / "ao2-cli" / "src" / "skill_contract_manifest.rs").write_text(
...
def test_no_factory_v3_guard_rejects_source_root_in_unrelated_rust_module(tmp_path):
    repo = tmp_path / "repo"
    (repo / "scripts").mkdir(parents=True)
    (repo / "crates" / "ao2-cli" / "src").mkdir(parents=True)
    (repo / "crates" / "ao2-runtime" / "src").mkdir(parents=True)
    (repo / "package.json").write_text('{"scripts": {}}\n', encoding="utf-8")
    (repo / "crates" / "ao2-cli" / "src" / "unrelated.rs").write_text(
        'let root = "../factory-v3";\n',
        encoding="utf-8",
    )
    subprocess.run(["git", "init", "-q"], cwd=repo, check=True)
    ...
    assert report["failures"][0]["path"] == "crates/ao2-cli/src/unrelated.rs"
...
def test_release_train_manifest_centralizes_stable_and_next_patch_defaults():
    ...
    assert (
        ...
        == "promote-stable-v0.5.13-v0.1.19"
    )
    assert (
    ...
    assert exported["AO2_RELEASE_TRAIN_AO2_TAG"] == "v0.5.13"
    assert exported["AO2_RELEASE_TRAIN_AO2_VERSION"] == "0.5.13"
    ...
    assert exported["AO2_RELEASE_TRAIN_PUBLIC_OPERATOR_CONFIRM"] == (
        "public-release-reviewed-v0.5.13-v0.1.19"
    )
    ...
    for workflow_path in [
        ...
        assert "default: v0.1.16" not in workflow
    ...
    assert "default: 0.5.0" not in post_stable
    assert "default: 0.5.0" not in windows_smoke
...
""",
    ...
    assert "promotion-plan.json" in summary["components"][0]["expected_assets"]
    assert "ao2-9.9.1-linux-aarch64.tar.gz" not in summary["components"][0]["expected_assets"]
...
def test_candidate_patch_release_rehearsal_audit_contract(tmp_path):
    ...
    assert audit["release_targets"]["ao2"]["tag"] == "v0.5.7"
...
def test_release_train_manifest_parity_audit_contract(tmp_path):
    ...
    assert summary["stable"]["ao2"]["tag"] == "v0.5.13"
    assert summary["next_patch"]["ao2"]["tag"] == "v0.5.13"
...
def test_release_immutability_audit_composes_stable_asset_and_download_checks():
    ...
    assert "v0.5.13" in readme
    assert "ao2-0.5.13-linux-x86_64.tar.gz" in readme
...
set -eu
if [ "${1:-}" = "version" ] && [ "${2:-}" = "--json" ]; then
  printf '{"package":"ao2","version":"0.5.13","target":"linux-x86_64","release_manifest_schema":"ao2.release-manifest.v1"}\n'
  exit 0
fi
if [ "${1:-}" = "--help" ]; then
  printf 'ao2 help\n'
...
""",
    ...
    assert summary["release_pair"]["ao2"]["tag"] == "v0.5.13"
