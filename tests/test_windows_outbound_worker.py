...
def test_ao2_full_profile_uses_windows_ci_partitions_instead_of_monolithic_workspace(
    ...
    assert any("cargo.exe test -p ao2-cli" in command and "cli_factory_queue" in command for command in joined)
    assert not any("cli_approval_replay cli_factory_queue" in command for command in joined)
    ...
    assert any("cargo.exe test -p ao2-cli" in command and "cli_memory" in command for command in joined)
    ...
    assert any(
        "cargo.exe test -p ao2-cli" in command and "cli_provider_run_repair" in command
        for command in joined
    )
    assert not any("cli_approval_replay" in command for command in joined)
    ...
    assert not any("cli_approval_replay cli_workbench_memory" in command for command in joined)
    assert any(
        "cargo.exe test -p ao2-cli" in command and "cli_workbench_lists" in command
        for command in joined
    )
    assert not any("cli_approval_replay cli_workbench_lists" in command for command in joined)
    assert any(
        "cargo.exe test -p ao2-cli" in command and "cli_workbench_api" in command
        for command in joined
    )
    assert not any("cli_approval_replay cli_workbench_api" in command for command in joined)
    assert any(
        "cargo.exe test -p ao2-cli" in command and "cli_workbench_serve" in command
        for command in joined
    )
    assert not any("cli_approval_replay cli_workbench_serve" in command for command in joined)
    assert any(
        "cargo.exe test -p ao2-cli" in command and "cli_workbench_operator" in command
        for command in joined
    )
    assert not any("cli_approval_replay cli_workbench_operator" in command for command in joined)
    assert any(
        "cargo.exe test -p ao2-cli" in command and "cli_workbench_launch" in command
        for command in joined
    )
    assert not any("cli_approval_replay cli_workbench_launch" in command for command in joined)
    assert any(
        "cargo.exe test -p ao2-cli" in command and "cli_workbench_greenfield" in command
        for command in joined
    )
    assert not any("cli_approval_replay cli_workbench_greenfield" in command for command in joined)
    assert any(
        "cargo.exe test -p ao2-cli" in command and "cli_workbench_obligation" in command
        for command in joined
    )
    assert not any("cli_approval_replay cli_workbench_obligation" in command for command in joined)
    assert any(
        "cargo.exe test -p ao2-cli" in command and "cli_workbench_evidence" in command
        for command in joined
    )
    assert not any("cli_approval_replay cli_workbench_evidence" in command for command in joined)
    assert any(
        "cargo.exe test -p ao2-cli" in command and "cli_workbench_export" in command
        for command in joined
    )
    assert not any("cli_approval_replay cli_workbench_export" in command for command in joined)
    assert any(
        "cargo.exe test -p ao2-cli" in command and "cli_workbench_factory" in command
        for command in joined
    )
    assert not any("cli_approval_replay cli_workbench_factory" in command for command in joined)
    assert any(
        "cargo.exe test -p ao2-cli" in command and "cli_workbench_support" in command
        for command in joined
    )
    assert not any("cli_approval_replay cli_workbench_support" in command for command in joined)
    assert any(
        "cargo.exe test -p ao2-cli" in command and "cli_workbench_release" in command
        for command in joined
    )
    assert not any("cli_approval_replay cli_workbench_release" in command for command in joined)
    assert any(
        "cargo.exe test -p ao2-cli" in command and "cli_workbench_run_evidence" in command
        for command in joined
    )
    assert not any("cli_approval_replay cli_workbench_run_evidence" in command for command in joined)
    assert any(
        "cargo.exe test -p ao2-cli" in command and "cli_workbench_provider" in command
        for command in joined
    )
    ...
    for project_start_suite in [
        ...
        assert any(
            "cargo.exe test -p ao2-cli" in command and project_start_suite in command
            for command in joined
