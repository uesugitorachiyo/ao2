...
use factory_project_contract::factory_project_start_bundle_verify_trust_boundary;
...
use release_comparison::{
    ...
    release_evidence_bundle_relative_path_allowed, release_evidence_bundle_secret_marker_failures,
...
struct FactoryProjectStartOptions<'a> {
    project_spec: &'a Path,
    project_root: &'a Path,
    run_id: String,
    verifier_command: String,
...
struct FactoryReplacementPacketOptions<'a> {
    queue_status: &'a Path,
    latest_queue_status: &'a Path,
    closure: &'a Path,
    closure_verification: &'a Path,
    cross_os_readbacks: &'a [PathBuf],
    out: &'a Path,
}
...
fn factory_project_start_bundle_json(
...
fn factory_project_start_closure_json(
...
fn factory_project_start_closure_verify_json(bundle_path: &Path) -> Result<serde_json::Value> {
...
fn factory_replacement_packet_json(
...
fn factory_replacement_packet_verify_json(bundle_path: &Path) -> Result<serde_json::Value> {
    let mut failures = Vec::<serde_json::Value>::new();
    let bundle_sha256 = match sha256_file(bundle_path) {
        Ok(sha256) => sha256,
        Err(error) => {
...
fn factory_project_start_bundle_verify_json(bundle_path: &Path) -> Result<serde_json::Value> {
    let mut failures = Vec::<serde_json::Value>::new();
    let bundle_sha256 = match sha256_file(bundle_path) {
        Ok(sha256) => sha256,
        Err(error) => {
