...
#[test]
fn cli_doctor_discovers_verified_custom_install_from_path() {
    let temp = tempfile::tempdir().unwrap();
    ...
    let doctor = ao2_with_env(
        ...
        [("HOME", home.to_str().unwrap()), ("PATH", path.as_str())],
    );
    assert!(doctor.status.success(), "{}", stderr(&doctor));
    let json: serde_json::Value = serde_json::from_str(&stdout(&doctor)).unwrap();
    assert_eq!(json["status"], "ok");
...
fn cli_doctor_prefers_custom_path_install_over_existing_default_install() {
    ...
    let custom_binary = custom_dir.join(binary_name);
    fs::copy(env!("CARGO_BIN_EXE_ao2"), &custom_binary).unwrap();
    fs::copy(env!("CARGO_BIN_EXE_ao2"), default_dir.join(binary_name)).unwrap();
    fs::write(
        custom_dir.join(format!("{binary_name}.install-verification.json")),
        serde_json::to_vec_pretty(&serde_json::json!({
            "schema_version": "ao2.install-verification-evidence.v1",
            "status": "verified",
            "install_status": "installed",
            "version": env!("CARGO_PKG_VERSION"),
            "target": target,
            "installed_binary": custom_binary,
            "offline_verification": {
                "schema_version": "ao2.release-archive-offline-verification.v1",
                "status": "verified",
                "checksum_coverage_verified": true,
    ...
    let path = prepend_path(&custom_dir);
    let doctor = ao2_with_env(
        [
            "doctor",
            "--json",
        ...
        [
            ...
            ("LOCALAPPDATA", home.to_str().unwrap()),
            ("PATH", path.as_str()),
        ],
    );
    assert!(doctor.status.success(), "{}", stderr(&doctor));
    let json: serde_json::Value = serde_json::from_str(&stdout(&doctor)).unwrap();
    assert_eq!(json["status"], "ok");
    assert_eq!(json["install"]["install_dir"], custom_dir.to_str().unwrap());
}

#[test]
fn cli_doctor_rejects_custom_path_binary_with_copied_install_evidence() {
    let temp = tempfile::tempdir().unwrap();
    let verified_dir = temp.path().join("verified-bin");
    let substituted_dir = temp.path().join("substituted-bin");
    let home = temp.path().join("home");
    let missing_provenance = temp.path().join("missing-provenance");
    fs::create_dir_all(&verified_dir).unwrap();
    fs::create_dir_all(&substituted_dir).unwrap();
    fs::create_dir_all(&home).unwrap();
    ...
    let verified_binary = verified_dir.join(binary_name);
    let substituted_binary = substituted_dir.join(binary_name);
    fs::copy(env!("CARGO_BIN_EXE_ao2"), &verified_binary).unwrap();
    fs::copy(env!("CARGO_BIN_EXE_ao2"), &substituted_binary).unwrap();
    fs::write(
        substituted_dir.join(format!("{binary_name}.install-verification.json")),
        serde_json::to_vec_pretty(&serde_json::json!({
            "schema_version": "ao2.install-verification-evidence.v1",
            "status": "verified",
            "install_status": "installed",
            "version": env!("CARGO_PKG_VERSION"),
            "target": target,
            "installed_binary": verified_binary,
            "offline_verification": {
                "schema_version": "ao2.release-archive-offline-verification.v1",
                "status": "verified",
                "checksum_coverage_verified": true,
    ...
    let path = prepend_path(&substituted_dir);
    let doctor = ao2_with_env(
        [
            "doctor",
            "--json",
    ...
    assert_eq!(json["status"], "attention");
    assert_eq!(
        json["install"]["verification_evidence"]["binary_path_matches"],
        false
    );
}
