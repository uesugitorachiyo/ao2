...
#[test]
fn cli_workbench_factory_project_start_next_action_api_reuses_read_only_preview() {
    let temp = tempfile::tempdir().unwrap();
    let repo = temp.path().join("factory-target");
    init_git_repo(&repo);
...
#[test]
fn cli_workbench_factory_project_start_operator_record_api_requires_operator_and_delegates_publish()
{
    let temp = tempfile::tempdir().unwrap();
    let repo = temp.path().join("factory-target");
    init_git_repo(&repo);
...
#[test]
fn cli_workbench_factory_project_start_operator_record_smoke_covers_next_action_to_publish() {
    let temp = tempfile::tempdir().unwrap();
    let repo = temp.path().join("factory-target");
    init_git_repo(&repo);
    let spaced_root = temp.path().join("AI Agent Teams").join("ao-2");
...
#[test]
fn cli_workbench_factory_project_start_hermes_flow_contract_api_delegates_to_contract_producer() {
    let temp = tempfile::tempdir().unwrap();
    let repo = temp.path().join("factory-target");
    init_git_repo(&repo);
...
"#,
    ...
    assert!(html.contains("AO2 Workbench"));
    assert!(html.contains("Run Queue"));
    assert!(html.contains("workbench-demo"));
    assert!(html.contains("Provider Health"));
    assert!(html.contains("Provider Readiness"));
    assert!(html.contains("Provider Score"));
    assert!(html.contains("provider-score-ready"));
    assert!(html.contains("ao2.provider-evidence-scorecard.v1"));
    assert!(html.contains("Provider Safety Warnings"));
    assert!(html.contains("role_contract_discovery"));
    assert!(html.contains("ao2_auto_loaded_role_contracts"));
    assert!(html.contains("provider-warning-output"));
    assert!(html.contains("provider-readiness-data"));
    assert!(html.contains("Run Provider Smoke"));
    assert!(html.contains("provider-smoke-output"));
    assert!(html.contains("provider-smoke-live-provider"));
    assert!(html.contains("sandbox_copy_then_digest_patch"));
    assert!(html.contains("900s"));
    assert!(html.contains("provider cannot write target repo directly"));
    assert!(html.contains("Provider Contracts"));
    assert!(html.contains("phase_1"));
    assert!(html.contains("AO2_LIVE_CODEX_SMOKE"));
    assert!(html.contains("Provider Contract Verification"));
    assert!(html.contains("contract-verification-output"));
    assert!(html.contains("ao2.provider-contract-verification.v1"));
    assert!(html.contains("status=verified"));
    assert!(html.contains("Release Health"));
    assert!(html.contains("release-health-release"));
    assert!(html.contains("release-health-asset-dir"));
    assert!(html.contains("release-health-provenance-dir"));
    assert!(html.contains("release-health-output"));
    assert!(html.contains("release-rollback-health"));
    assert!(html.contains("/api/release-health"));
    assert!(html.contains("ao2 doctor --json --release"));
    assert!(html.contains("renderReleaseRollbackHealth"));
    assert!(html.contains("rollback_status="));
    assert!(html.contains("macos-aarch64"));
    assert!(html.contains("linux-x86_64"));
    assert!(html.contains("windows-x86_64"));
    assert!(html.contains("Release History"));
    assert!(html.contains("release-history-dir"));
    assert!(html.contains("release-history-refresh"));
    assert!(html.contains("release-history-export"));
    assert!(html.contains("release-history-output"));
    assert!(html.contains("/api/release-history"));
    assert!(html.contains("renderReleaseHistory"));
    assert!(html.contains("Release Comparison Bundle"));
    assert!(html.contains("release-comparison-out-dir"));
    assert!(html.contains("release-comparison-generate"));
    assert!(html.contains("release-comparison-verify"));
    assert!(html.contains("release-comparison-latest"));
    assert!(html.contains("release-comparison-export"));
    assert!(html.contains("release-comparison-verification"));
    assert!(html.contains("release-comparison-output"));
    assert!(html.contains("release-retention-keep-releases"));
    assert!(html.contains("release-retention-keep-bundles"));
    assert!(html.contains("release-retention-preview"));
    assert!(html.contains("release-retention-prune"));
    assert!(html.contains("release-retention-output"));
    assert!(html.contains("/api/release-comparison"));
    assert!(html.contains("/api/release-comparison/latest"));
    assert!(html.contains("/api/release-retention/prune"));
    assert!(html.contains("renderReleaseComparisonVerification"));
    assert!(html.contains("Project-Start Next Action"));
    assert!(html.contains("project-start-next-action-run-id"));
    assert!(html.contains("project-start-next-action-out-dir"));
    assert!(html.contains("project-start-next-action-contract"));
    assert!(html.contains("project-start-next-action-refresh"));
    assert!(html.contains("project-start-next-action-output"));
    assert!(html.contains("/api/factory/project-start/next-action"));
    assert!(html.contains("refreshProjectStartNextAction"));
    assert!(html.contains("project-start-operator-record-form"));
    assert!(html.contains("project-start-operator-record-run-id"));
    assert!(html.contains("project-start-operator-record-out-dir"));
    assert!(html.contains("project-start-operator-record-contract"));
    assert!(html.contains("project-start-operator-record-record-out"));
    assert!(html.contains("project-start-operator-record-publish"));
    assert!(html.contains("project-start-operator-record-output"));
    assert!(html.contains("/api/factory/project-start/operator-record"));
    assert!(html.contains("publishProjectStartOperatorRecord"));
    assert!(html.contains("Project-Start Hermes Flow Contract"));
    assert!(html.contains("project-start-hermes-flow-contract-form"));
    assert!(html.contains("project-start-hermes-flow-contract-out"));
    assert!(html.contains("project-start-hermes-flow-contract-refresh"));
    assert!(html.contains("project-start-hermes-flow-contract-output"));
    assert!(html.contains("/api/factory/project-start/hermes-flow-contract"));
    assert!(html.contains("refreshProjectStartHermesFlowContract"));
    assert!(html.contains("Task Templates"));
    assert!(html.contains("ao2 upgrade apply --github-release"));
}
