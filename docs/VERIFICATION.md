...
- `Candidate Patch Release Rehearsal` in
  ...
  It validates the manifest-backed `v0.5.1` / `v0.1.16` candidate targets in
...
- `npm run release:train-drill`: rehearses release evidence closure, the
  ...
  `AO2_RELEASE_TRAIN=next_patch` rehearses the `v0.5.1` / `v0.1.16` candidate
...
- `Stable Release Promotion`: manual GitHub Actions workflow that consumes the
  ...
  `promote-stable-v0.5.1-v0.1.16` confirmation string to allow the underlying
...
- `npm run release:stable-promotion-operator-checklist`: converts a passed
  ...
  `promote-stable-v0.5.1-v0.1.16` confirmation string, but does not enter it.
