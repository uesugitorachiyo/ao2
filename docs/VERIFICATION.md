...
- `Candidate Patch Release Rehearsal` in
  ...
  It validates the manifest-backed `v0.5.1` / `v0.1.15` candidate targets in
...
- `npm run release:public-pair-digest-audit`: read-only post-release verifier
  ...
  `ao2-0.5.1-linux-aarch64.tar.gz`, `ao2-0.5.1-linux-x86_64.tar.gz`,
  `ao2-0.5.1-macos-aarch64.tar.gz`, and
  `ao2-0.5.1-windows-x86_64.tar.gz`, plus control-plane
...
- `npm run release:train-drill`: rehearses release evidence closure, the
  ...
  `AO2_RELEASE_TRAIN=next_patch` rehearses the `v0.5.1` / `v0.1.15` candidate
...
- `Stable Release Promotion`: manual GitHub Actions workflow that consumes the
  ...
  `promote-stable-v0.5.1-v0.1.15` confirmation string to allow the underlying
...
- `npm run release:stable-promotion-operator-checklist`: converts a passed
  ...
  `promote-stable-v0.5.1-v0.1.15` confirmation string, but does not enter it.
