...
```sh
ao2 install update \
  --archive dist/ao2-0.4.79-macos-aarch64.tar.gz \
...
```sh
ao2 install update \
  --archive dist-linux-x86_64/ao2-0.4.79-linux-x86_64.tar.gz \
  --provenance-dir dist-provenance
ao2 version --json
ao2 doctor --json
ao2 provider matrix --json
...
```sh
ao2 install update \
  --archive dist-linux/ao2-0.4.79-linux-aarch64.tar.gz \
  --provenance-dir dist-provenance
ao2 version --json
ao2 doctor --json
ao2 provider matrix --json
...
ao2.exe install update `
  --archive dist-windows\ao2-0.4.79-windows-x86_64.tar.gz `
  --provenance-dir dist-provenance
...
```sh
ao2 install update \
  --release-base-url https://github.com/uesugitorachiyo/ao2/releases/download/v0.4.79
```
...
ao2 upgrade apply \
  ...
  --asset-dir target/release-download/v0.4.79
```
...
ao2 doctor --json \
  --release v0.4.79 \
  --release-asset-dir target/release-download/v0.4.79 \
  --provenance-dir target/release-download/v0.4.79
```
...
digests, and a canonical bundle digest in `SHA256SUMS` so the
ao2-control-plane offline verifier can check the same handoff. Verification
fails closed if replay was not accepted, digest failures are present, report
contract verification is missing or failed, operator evidence is missing,
candidate-correlation triage is absent from the release assembly, readiness,
