...
- `ao2-0.5.10-macos-aarch64.tar.gz`
- `ao2-0.5.10-linux-x86_64.tar.gz`
- `ao2-0.5.10-windows-x86_64.tar.gz`
...
```sh
mkdir -p ao2-stable && cd ao2-stable
base_url="https://github.com/uesugitorachiyo/ao2/releases/download/v0.5.10"
curl -fLO "$base_url/SHA256SUMS"
curl -fLO "$base_url/ao2-0.5.10-macos-aarch64.tar.gz"
grep '  ao2-0.5.10-macos-aarch64.tar.gz$' SHA256SUMS > SHA256SUMS.selected
...
```sh
tar -xzf ao2-0.5.10-<platform>.tar.gz
./verify-release.sh
...
AO2 `v0.5.10` publishes aggregate checksums and GitHub attestations, but not the
detached RSA provenance files required by the `v0.5.10` `install update`
...
```sh
ao2 install update \
  --archive ao2-0.5.10-<platform>.tar.gz \
...
[`v0.5.10` release](https://github.com/uesugitorachiyo/ao2/releases/tag/v0.5.10)
...
The current stable public release line is `v0.5.10`.
